...
public class J8As {
	...
	public static PriorityQueue<NodeAs> fila = new PriorityQueue<>();
	public static HashSet<NodeAs> visitados = new HashSet<>();

	public static NodeAs procura(int[][] solucao, NodeAs no) {
		fila.add(no);
		while (!fila.isEmpty()) {
			NodeAs novo_no = fila.remove();
	...
	public static void expandir(NodeAs no, int[][] solucao) {
		NodeAs no1 = new NodeAs();
		...
		if (no.pos_branca == 0 && no.jogada == 'c') {
			...
			no1.jogada = 'd';
			no1.pos_branca = 1;
			no1.custo = calcCUSTO(no1.estado, solucao) + no.profundidade;
			...
			no2.jogada = 'b';
			no2.pos_branca = 3;
			no2.custo = calcCUSTO(no2.estado, solucao) + no.profundidade;
			...
			no3.jogada = 'b';
			no3.pos_branca = 4;
			no3.custo = calcCUSTO(no3.estado, solucao) + no.profundidade;
			...
			no4.jogada = 'd';
			no4.pos_branca = 5;
			no4.custo = calcCUSTO(no4.estado, solucao) + no.profundidade;
	...
	public static void caminho(NodeAs no) {
	...
	public static void main(String[] args) {
		...
		NodeAs no = new NodeAs(estado_inicial, null, 'n', 0, posBRANCO(estado_inicial),
			calcCUSTO(estado_inicial, solucao));
		NodeAs no_final = procura(solucao, no);
		if (no_final != null) {
			...
			System.out.printf("Memória utilizada: %.3f MB (%.3f KB)%n", used_memory / (1024 * 1024),
				used_memory / 1024);
